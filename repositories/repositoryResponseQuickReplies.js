...
exports.get_quickReplies_location = function (res, context) {
  ...
  if (!response.quick_replies.length) {
    response = {
      text: `Lo siento! No hay lugares donde pasen ${context.data.movie}`,
    };
  }
...
exports.get_quickReplies_date = function (res, context) {
  //get database from container
  const database = this.database;
  const response = {...res};
...
exports.get_quickReplies_date_place = function (res, context) {
  //get database from container
  const database = this.database;
  const response = {...res};
  ...
  if (!response.quick_replies.length) {
    response = {
      text: `Lo siento! No hay horarios para ${context.data.movie} en ${context.data.place}`,
    };
  }
  return response;
};
