...
describe ('Template movies', () => {
  describe ('#Movies from 0 - 8', () => {
    ...
    it ('should return movies from 0 to 8 and "Ver mas" button', () => {
      var database_movie_titles = database_file_test.movies
        .slice (0, 9)
        .map (movie => movie.content.title);
      ...
      var database_movie_subtitles = database_file_test.movies
        .slice (0, 9)
        .map (movie => movie.content.synopsis);
      response.attachment.payload.elements.forEach ((elem, index) => {
        if (index < response.attachment.payload.elements.length - 1) {
          assert.notEqual (
            database_movie_subtitles.indexOf (elem.subtitle),
            -1
          );
        }
      });

      var database_movie_posterUrl = database_file_test.movies
        .slice (0, 9)
        .map (movie => movie.content.posterUrl);
      response.attachment.payload.elements.forEach ((elem, index) => {
        if (index < response.attachment.payload.elements.length - 1) {
          assert.notEqual (
            database_movie_posterUrl.indexOf (elem.image_url),
            -1
          );
        }
      });
...
describe ('Template movies place', () => {
  ...
  it ('should return movies for Montevideo Shopping for 2018-03-13', () => {
    ...
    database_file_test.schedules.contentCinemaShows.forEach (contentCinema => {
      contentCinema.cinemaShows.forEach (cinemaShow => {
        cinemaShow.shows.some (show => {
          ...
          if (
            show.cinemaName.toLowerCase () === 'movie montevideo' &&
            show_date === user_date
          ) {
    ...
    const place_movies_date_title = database_file_test.movies_id
      .filter (e => place_movies_date.indexOf (e.id) !== -1)
      .map (e => e.title);
    response.attachment.payload.elements.forEach ((elem, index) => {
      if (index < response.attachment.payload.elements.length - 1) {
    ...
    database_file_test.schedules.contentCinemaShows.forEach (contentCinema => {
      contentCinema.cinemaShows.forEach (cinemaShow => {
        cinemaShow.shows.some (show => {
          ...
          var user_date = moment ('2018-03-13').dayOfYear ();
          if (
            show.cinemaName.toLowerCase () === 'movie portones' &&
            show_date === user_date
          ) {
            place_movies_date.push (contentCinema.contentId);
            return true;
    ...
    database_file_test.schedules.contentCinemaShows.forEach (contentCinema => {
      contentCinema.cinemaShows.forEach (cinemaShow => {
        cinemaShow.shows.some (show => {
          ...
          var user_date = moment ('2018-03-13').dayOfYear ();
          if (
            show.cinemaName.toLowerCase () === 'movie nuevocentro' &&
            show_date === user_date
          ) {
            place_movies_date.push (contentCinema.contentId);
            return true;
    ...
    database_file_test.schedules.contentCinemaShows.forEach (contentCinema => {
      contentCinema.cinemaShows.forEach (cinemaShow => {
        cinemaShow.shows.some (show => {
          ...
          var user_date = moment ('2018-03-13').dayOfYear ();
          if (
            show.cinemaName.toLowerCase () === 'movie punta carretas' &&
            show_date === user_date
          ) {
            place_movies_date.push (contentCinema.contentId);
            return true;
...
describe ('Template movies genre', () => {
  ...
  it ('should return movies for 2018-03-13 that are of the genre "Acción" ', () => {
    ...
    const movies_accion_date = database_file_test.movies
      .filter (movie => {
        return (
          movie.content.genre
            .split (', ')
    ...
    const movies_accion_date_title = database_file_test.movies_id
      .filter (e => movies_accion_date.indexOf (e.id) !== -1)
      .map (e => e.title);
